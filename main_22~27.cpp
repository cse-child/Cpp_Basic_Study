...
int main() {
	...
	// 주소의 단위: Byte
	// 주소는 정수표현이기 때문에 bit단위로는 표현될 수 없다.

	// 포인터 변수를 int*로 선언하였으면 해당 주소의 데이터는 int형이어야만 한다.
	// 포인터는 이미 답이 정해진 변수이다. 답정너임
	// int형 포인터변수를 선언한 것은 해당 주소에 int형 데이터만 넣겠다.라고
	// 미리 정해 놓은 것이다.
	
	// 그렇다면 float 변수도 4byte이고 int형도 4byte니까 float을 int*에 저장하면?
	float f = 3.f;
	int* pInt2 = (int*)&f; // int형 포인터로 형변환 해주지 않으면 에러남
	 
	i = *pInt2;
	// i 출력 결과: 1077936128
	// 변수 f 에는 실수 표현방식(부동소수점)의 3이 들어있다. (OO.OOOOOOOO~ 32bit로 이루어짐)
	// 그러나 pInt2는 int형이기 때문에 int관점에서 부동소수점을 해석하면 매우 큰 숫자가 되는 것이다.
